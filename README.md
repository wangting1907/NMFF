...
def train(local_rank, args):
    ...
    model = Siren(in_features=args.embed_length, out_features=3, hidden_features=128,
    ...
    train_data_dir = './dataset_real_samson.mat'
    val_data_dir = './dataset_real_samson.mat'
    ...
    data_h=scio.loadmat('dataset_real_samson.mat')
    H=data_h.get("H")                                                  #read abundance
   # H=H.reshape(3,95,95)
    abundance=torch.Tensor(H)
    if args.eval_only:
        print('Evaluation ...')
    ...
    for epoch in range(args.start_epoch, total_epochs):
        ...
        for i, (data,  norm_idx) in enumerate(train_dataloader):
            ...
            output_list=endab(Endmember,abundance)                                                                       #######
            ...
            if i % args.print_freq == 0 or i == len(train_dataloader) - 1:
                ...
                print_str = '[{}] Rank:{}, Epoch[{}/{}], Step [{}/{}], lr:{:.2e} loss_sum: {}, PSNR: {}, MSSSIM: {}'.format(
                    time_now_string, local_rank, epoch+1, args.epochs, i+1, len(train_dataloader), lr, loss_sum,
        ...
        if (epoch + 1) % args.eval_freq == 0 or epoch > total_epochs - 10:
            ...
            val_psnr, val_msssim, Endmembers= evaluate(model, val_dataloader, PE, local_rank, abundance, args)
            val_end_time = datetime.now()
            if args.distributed and args.ngpus_per_node > 1:
                val_psnr = all_reduce([val_psnr.to(local_rank)])
                val_msssim = all_reduce([val_msssim.to(local_rank)]) 
           #     Endmembers = all_reduce([Endmembers.to(local_rank)])          
       # torch.save(Endmembers, '{}/Endmembers.pth'.format(args.outf))
    print("Training complete in: " + str(datetime.now() - start))

@torch.no_grad()
def evaluate(model, val_dataloader, pe, local_rank,abundance, args):
    psnr_list = []
    msssim_list = []
    if args.dump_images:
